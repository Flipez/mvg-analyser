    ...
    class Clickhouse
      ...
      def setup_tables
        connection.create_table("responses",
          ...
          t << "datestring Date CODEC(Delta, ZSTD(3))"
          t << "timestamp DateTime CODEC(Delta, ZSTD(3))"
          t << "station LowCardinality(String) CODEC(ZSTD(3))"
          t << "plannedDepartureTime DateTime CODEC(Delta, ZSTD(3))"
          t << "realtime Boolean CODEC(ZSTD(3))"
          t << "delayInMinutes Int32 CODEC(ZSTD(3))"
          t << "realtimeDepartureTime DateTime CODEC(Delta, ZSTD(3))"
          ...
          t << "platform Int32 CODEC(ZSTD(3))"
          t << "platformChanged Boolean CODEC(ZSTD(3))"
          t << "stopPositionNumber Int32 CODEC(ZSTD(3))"
